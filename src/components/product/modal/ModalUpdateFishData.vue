...
const handleSave = async () => {
  ...
  if (props.existingRecord && props.existingRecord._id && props.productId) {
    // Update existing record
    updateFishGrowthHistory({
      _id: props.existingRecord._id,
...
const { mutate: addFishGrowthHistory, isPending: isAddingFishGrowthHistory } = useMutation({
  ...
  onSuccess: (data: any, payload: IFishGrowthHistoryPayload) => {
    if (data.data) {
      ...
      toast.error(data.error?.message || 'ไม่สามารถบันทึกข้อมูลได้')
    }
  },
  onError: (error: any) => {
    console.error(error)
...
const { mutate: updateFishGrowthHistory, isPending: isUpdatingFishGrowthHistory } = useMutation({
  ...
  onSuccess: (data: any, payload: IUpdateFishGrowthHistoryPayload) => {
    if (data.data) {
      ...
      toast.error(data.error?.message || 'ไม่สามารถแก้ไขข้อมูลได้')
    }
  },
  onError: (error: any) => {
    console.error(error)
